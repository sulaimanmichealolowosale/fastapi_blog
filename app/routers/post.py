...
def create_post(post: schemas.CreatePost, db: Session = Depends(get_db),
    ...
    if category is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"A category with id: {post.category_id} does not exists ")
...
def get_single_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post).filter(models.Post.id == id,
                                        models.Post.published == True).first()
...
def update_post(post: schemas.CreatePost, id: int, db: Session = Depends(get_db),
                current_user: int = Depends(oauth2.get_current_admin_user)):
    ...
    if post_result.owner_id is not current_user.id:
